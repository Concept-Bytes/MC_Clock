...
# Debug configuration
debug_hours = 6  # Set this to the desired debug hour (0-23)
debug_minutes = 0  # Set this to the desired debug minute (0-59)
...
while running:
    ...
    if DEBUG:
        hours = debug_hours + debug_minutes / 60  # Use debug time if debug mode is enabled
    else:
        now = datetime.now()
        hours = now.hour + now.minute / 60  # Include minutes as a fraction of an hour

    # Calculate angle where 12:00 PM (noon) = 0 degrees and 12:00 AM (midnight) = 180 degrees
    if hours >= 12:
        hours -= 12  # Convert 24-hour format to 12-hour format
    
    angle = (hours / 24) * 360  # Map 0-12 hours to 0-360 degrees
    angle = 360 - angle  # Reverse direction to match the desired rotation (noon = 0, midnight = 180)
